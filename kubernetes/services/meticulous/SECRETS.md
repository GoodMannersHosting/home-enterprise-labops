...
Because `spec.bootstrap.initdb.secret.name` points at **`meticulous-db-owner`**, CloudNativePG **does not** create a separate `<cluster>-app` secret for bootstrap. Application connection strings are stored separately in **`meticulous-db-uri`** (below).
...
## 1b. `meticulous-db-uri` (required for met-api / met-controller)

| Key   | Description |
| ----- | ----------- |
| `uri` | `postgres://…@meticulous-db-rw:5432/meticulous` for **`MET_DATABASE__URL`** (password URL-encoded; generated by the same `seal-meticulous-secrets.sh` run as db-owner, using the same owner password).

**Sealed Secrets** encrypts values for a specific target `Secret` name. After moving `uri` to **`meticulous-db-uri`**, regenerate **`secret.meticulous-db-uri.yaml`** and **`secret.meticulous-db-owner.yaml`** with the **current** owner password. To avoid rotating JWT/S3 keys, skip re-sealing runtime:

```bash
METICULOUS_DB_OWNER_PASSWORD='(current owner password)' \
  SKIP_METICULOUS_RUNTIME_SEAL=1 ./seal-meticulous-secrets.sh
```

Commit **`secret.meticulous-db-owner.yaml`** and **`secret.meticulous-db-uri.yaml`** (leave **`secret.meticulous-runtime.yaml`** unchanged unless you intend to rotate those keys).
...
Committed sealed YAML: **`meticulous-db-owner`**, **`meticulous-db-uri`**, **`meticulous-runtime`**.
...
After sealing, keep **`kustomization.yaml`** resources in order: **`secret.meticulous-nats.yaml`** (-4), **`secret.meticulous-db-owner.yaml`** (-3), **`secret.meticulous-db-uri.yaml`** (-3), **`secret.meticulous-runtime.yaml`** (-2), then **`cnpg-cluster.yaml`**. Commit only sealed YAML (never cleartext `Secret` objects).
...
For manual one-off runs (break-glass), build and use the optional image from the Meticulous repo `Dockerfile.sqlx-migrate` with `DATABASE_URL` set to the same `uri` as in **`meticulous-db-uri`**.
