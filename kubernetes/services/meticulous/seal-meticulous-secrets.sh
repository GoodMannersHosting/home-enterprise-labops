...
#
# Re-seal only DB owner + app URI (keep existing `meticulous-runtime` file unchanged):
#   METICULOUS_DB_OWNER_PASSWORD='…' SKIP_METICULOUS_RUNTIME_SEAL=1 ./seal-meticulous-secrets.sh
#
# Outputs (in this directory):
#   secret.meticulous-db-owner.yaml
#   secret.meticulous-db-uri.yaml
...
#   secret.meticulous-nats.yaml (-4) → db-owner (-3) → db-uri (-3) → runtime (-2) → CNPG cluster → ...
...
# rotate the DB user while re-sealing.
db_pass="${METICULOUS_DB_OWNER_PASSWORD:-$(openssl rand -base64 32)}"
# CNPG does not create `<cluster>-app` when `bootstrap.initdb.secret.name` is set. Apps read Postgres URL from
# `meticulous-db-uri` (key `uri`); CNPG bootstrap reads `username` / `password` from `meticulous-db-owner` only.
db_uri="$(
...
kubectl create secret generic meticulous-db-uri \
	--namespace "${NS}" \
	--from-literal=uri="${db_uri}" \
	--dry-run=client -o yaml | meticulous_seal_annotate_wave -3 | kubeseal "${METICULOUS_SEAL_KUBESEAL_ARGS[@]}" \
	>secret.meticulous-db-uri.yaml

if [[ -z "${SKIP_METICULOUS_RUNTIME_SEAL:-}" ]]; then
	runtime_from=(
...
echo "  ${SCRIPT_DIR}/secret.meticulous-db-uri.yaml"
if [[ -z "${SKIP_METICULOUS_RUNTIME_SEAL:-}" ]]; then
...
echo ""
echo "Next: ensure kustomization.yaml lists db-owner, db-uri, and runtime before cnpg-cluster.yaml."
